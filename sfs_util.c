...
/****************************************************************************/
/* prints number of free blocks and free inode entries
/*
/****************************************************************************/
...
void stats()
{
	...
	readSFS(1, (char *)buf1);
	for (i = 0; i < BLB; i++)
		blocks_free -= (buf1[i] - 48);


	readSFS(2, (char *)buf2);
	for (i = 0; i < INB; i++)
		inodes_free -= (buf2[i] - 48);
...
void display(char *fname)
{
	...
	if (found) { // display file contents to standard output 	
			...
			int arr[3];
			arr[0] = stoi(_inode_table[e_inode].XX,2);
			arr[1] = stoi(_inode_table[e_inode].YY,2);
			arr[2] = stoi(_inode_table[e_inode].ZZ,2);

			for (i=0; i<3; i++) {
				if (arr[i]==0) continue;
				
				char buf[1024];
				int print=readSFS(arr[i],(char *)buf);
...
/****************************************************************************/
/* creates a file with given name within the current directory, check for same name available space
/*
/****************************************************************************/

void create(char *fname)
{
	printf("\(Enter Max 3072 characters OR hit ESC-ENTER to end \)\n");
	...
	if (empty_dentry == -1 && empty_dblock == -1)
	{
		printf("Error: Maximum file entries reached.\n");
		return;
	}
	else
	{ // otherwise
		...
		for (int i = 0; i < 3; ++i)
		{
			if (arr[i][0])
			{
				...
				{
					// writing block
					writeSFS(temp_file_newblocks[i], arr[i]);
					switch (i)
					{ // update the inode entry of current dir to reflect that we are using a new block
					case 0:
		...
		strncpy(_directory_entries[empty_dentry].fname, fname, 252); 
...
/****************************************************************************/
/* removes file/directory, error if no file/directory exist
/*
/****************************************************************************/
